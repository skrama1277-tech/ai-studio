...
const METRICS = [
  ...
  { value: "112.75", label: "FTE repurposed", pillar: "cost", icon: "person" },
];
...
export default function App() {
  ...
  const filtered = useMemo(() => {
    let r = ucs;
    if (fDomain === "technology") r = r.filter(u => u.domain === "technology");
    else if (fDomain !== "all") r = r.filter(u => u.dept.toLowerCase().includes(fDomain.toLowerCase()));
  ...
  return (
    <div style={{ fontFamily:"'Inter','Segoe UI',system-ui,sans-serif", background:t.bg, minHeight:"100vh", color:t.tx1, transition:"background 0.3s,color 0.3s" }}>
      ...
      <div style={{ maxWidth:1160, margin:"0 auto", padding:"0 clamp(16px,3vw,40px)" }}>
        {view==="home" && <HomePage t={t} dk={dk} ucs={ucs} videos={videos} archs={archs} isAdmin={isAdmin} onUpload={handleUpload} onArchUpload={handleArchUpload} go={go} goRole={rc=>{setFD(rc.id);setSearch("");setFP(["all"]);go("catalog");}} />}
  ...
  return (
    <div style={{ paddingBottom:60 }}>
      <div className="fade" style={{ padding:"clamp(24px,4vw,48px) 0 clamp(20px,3vw,36px)", textAlign:"center" }}>
        <h1 style={{ fontSize:"clamp(32px,5vw,56px)", fontWeight:800, lineHeight:1.08, letterSpacing:"-0.03em", maxWidth:680, margin:"0 auto 14px" }}><span style={{ color:t.accent }}>QBE</span> AI Innovation Studio</h1>
      ...
      <div style={{ overflow:"hidden", borderTop:`1px solid ${t.bd}`, borderBottom:`1px solid ${t.bd}`, marginBottom:40, padding:"14px 0", background:t.bgMuted }}>
        <div style={{ display:"flex", width:"max-content", animation:"ticker 30s linear infinite" }} onMouseEnter={e=>e.currentTarget.style.animationPlayState="paused"} onMouseLeave={e=>e.currentTarget.style.animationPlayState="running"}>
          {[...Array(2)].flatMap((_,rep) => [
            ...
            { value: "94%", label: "CSAT score        (+9 pts uplift)", icon: "star", color: t.amber },
            { value: "112.75", label: "FTE repurposed", icon: "person", color: t.green },
...
function JourneyPage({ t, dk, go }) {
  ...
  return (
    <div style={{ padding:"28px 0 60px" }} className="fade">
      ...
      <div style={{ background:`linear-gradient(135deg,${dk?"#0A1E35":"#003057"}ee,${t.blue}cc)`, borderRadius:14, padding:"24px 28px", color:"#fff", marginBottom:26 }}><div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}><Icon name="bolt" size={17} color="#fff" /><span style={{ fontSize:17, fontWeight:700 }}>AI-First Talent Transformation</span></div><div style={{ fontSize:14, lineHeight:1.7, opacity:0.92 }}>QBE and Accenture are embedding AI capability at every level of the organisation — from executive GenAI literacy programmes to hands-on delivery teams co-building agentic solutions. This ensures AI adoption is sustained, governed and commercially impactful at scale.</div></div>
...
function DashboardPage({ t, dk, ucs, go }) {
  ...
  return (
    <div style={{ padding:"28px 0 60px" }} className="fade">
      ...
      <h2 style={{ fontSize:"clamp(26px,4vw,42px)", fontWeight:800, letterSpacing:"-0.02em", marginBottom:8 }}>AI Value Realization</h2>
  ...
  return (
    <div style={{ padding:"28px 0 60px" }} className="fade">
      ...
      <div style={{ display:"grid", gridTemplateColumns:"64px 1fr", gap:"8px 0", marginBottom:14, alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>{[["all","All"],["claims","Claims"],["underwriting","Underwriting"],["enterprise","Enterprise"],["technology","Technology"]].map(([v,l])=><button key={v} className={`tbtn${fDomain===v?" on":""}`} onClick={()=>setFD(v)}>{l}</button>)}</div>
...
function UCCard({ uc, t, dk, onClick, idx, onEdit, isAdmin, hasVideo, hasArch, onUpload, onArchUpload }) {
  ...
  return (
    <div onClick={onClick} className="card card-hover" style={{ padding:22, display:"flex", flexDirection:"column", animation:`fadeIn 0.3s ease ${Math.min(idx*0.03,0.3)}s both` }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8, gap:6 }}><span style={{ fontSize:12, color:t.tx3, fontWeight:500 }}>{uc.dept}</span><div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:3 }}><span style={{ background:sc.bg, color:sc.tx, fontSize:11, fontWeight:500, padding:"3px 11px", borderRadius:20, display:"flex", alignItems:"center", gap:5, whiteSpace:"nowrap" }}>{uc.status==="Live"&&<span style={{ width:5, height:5, borderRadius:"50%", background:sc.tx, animation:"pulse 2s infinite" }} />}{uc.status}</span>{uc.goLiveMonth&&<span style={{ fontSize:10, color:t.tx3, whiteSpace:"nowrap" }}>{uc.goLiveMonth}{uc.goLiveYear?`-${uc.goLiveYear}`:""}</span>}</div></div>
      ...
      <p style={{ fontSize:14, color:t.tx2, lineHeight:1.6, flex:1, marginBottom:uc.tools?.length?8:14 }}>{uc.summary}</p>
      {uc.tools?.length>0&&<div style={{ display:"flex", gap:4, flexWrap:"wrap", marginBottom:14 }}>{uc.tools.map(tool=><span key={tool} style={{ fontSize:10, background:t.accentBg, color:t.accent, borderRadius:20, padding:"2px 10px", border:`1px solid ${t.accentBd}`, fontWeight:600 }}>{tool}</span>)}</div>}
...
// ── DETAIL PAGE ───────────────────────────────────────────────────────────
function DetailPage({ t, dk, uc, videos={}, archs={}, isAdmin, onUpload, onArchUpload, onRemove, onRemoveArch, go, onEdit, onDelete }) {
  ...
  return (
    <div style={{ padding:"28px 0 60px", maxWidth:840 }} className="fade">
      <div style={{ display:"flex", gap:8, marginBottom:20 }}><button onClick={()=>go("catalog")} style={{ background:t.bgCard, border:`1px solid ${t.bd}`, borderRadius:20, padding:"7px 16px", fontSize:13, color:t.tx2, display:"flex", alignItems:"center", gap:5 }}><Icon name="back" size={14} color={t.tx2} />Back</button>{isAdmin&&<button onClick={onEdit} style={{ background:t.bgCard, border:`1px solid ${t.bd}`, borderRadius:20, padding:"7px 16px", fontSize:13, color:t.tx2, display:"flex", alignItems:"center", gap:5 }}><Icon name="edit" size={14} color={t.tx2} />Edit</button>}{isAdmin&&<button onClick={()=>{ if(window.confirm(`Delete "${uc.title}"?`)) onDelete(uc.id); }} style={{ background:t.bgCard, border:`1px solid ${t.bd}`, borderRadius:20, padding:"7px 16px", fontSize:13, color:t.tx2, display:"flex", alignItems:"center", gap:5 }}><Icon name="trash" size={14} color={t.tx2} />Delete</button>}</div>
...
function AdminModal({ t, uc, ucs, onSave, onDelete, onClose, videos, archs, onUpload, onArchUpload, onRemoveVideo, onRemoveArch }) {
  ...
  const load=id=>{const u=ucs.find(x=>x.id===id);if(!u)return;setForm({...u,pillars:u.pillars||(u.pillar?[u.pillar]:["speed"]),tools:u.tools||[],goLiveMonth:u.goLiveMonth||"",goLiveYear:u.goLiveYear||""});setOT((u.outcomes||[]).join("\n"));setFT((u.financial||[]).join("\n"));setOpT((u.operational||[]).join("\n"));setGT((u.governance||[]).join("\n"));setFsT((u.fromSteps||[]).join("\n"));setTsT((u.toSteps||[]).map(s=>(s.a?"AGENT: ":"")+s.l).join("\n"));};
  const toggleTool=tool=>setForm(p=>{const cur=p.tools||[];return{...p,tools:cur.includes(tool)?cur.filter(x=>x!==tool):[...cur,tool]};});
...
function Footer({ t, isAdmin }) {
  return <div style={{ borderTop:`1px solid ${t.bd}`, padding:"14px clamp(16px,3vw,40px)", display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:20 }}><span style={{ fontSize:12, color:t.tx3 }}>QBE AI Innovation Studio V5.0</span><div style={{ display:"flex", alignItems:"center", gap:10 }}>{isAdmin&&<span style={{ fontSize:11, color:t.accent, fontWeight:600, letterSpacing:"0.08em" }}>ADMIN SESSION</span>}</div></div>;
}